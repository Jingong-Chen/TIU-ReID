#!/usr/bin/env python
"""Baseline: Head-only unlearning (only modify classifier head)."""
...
def main():
    ...
    ap.add_argument("--lambda_forget", type=float, default=1.0, help="Forget loss weight")
    ...
    params = [{"params": classifier.parameters(), "lr": args.lr_head}]
    ...
    print(f"[INFO] Head-only unlearning (freeze base, only train classifier)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(loader, start=1):
            ...
            with amp.autocast(enabled=(device == "cuda")):
                ...
                # Forget loss: maximize entropy for target
                loss_forget = torch.tensor(0.0, device=device)
                if n_target > 0:
                    logits_target = logits[is_target == 1]
                    probs_target = F.softmax(logits_target, dim=1)
                    entropy = (-probs_target * (probs_target + 1e-12).log()).sum(dim=1)
                    max_entropy = np.log(num_classes)
                    loss_forget = (max_entropy - entropy).mean()

                loss = args.lambda_id * loss_id + args.lambda_tri * loss_metric + args.lambda_forget * loss_forget
